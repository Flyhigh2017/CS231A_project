...
class Detector(object):
    ...
    def interpret_output(self, output):
        ...
        scales[:,:,0] = output[:,:,4]
        scales = scales.reshape((480,1))
        class_probs = output[:,:,5:]
        softp = class_probs.reshape((480,3))

        sumd = np.sum(np.exp(softp),axis=1)
        sumd = sumd.reshape((480,1))
        #print sumd.shape
        ...
        for i in range(dim[0]):
            ...
            for j in range(dim[1]):
                ...
                if max_prob > 0.57: #or p2[i,j] > 0.9 or p3[i,j] > 0.9:
                    start_width = j * 64.0
                    x_center = (boxes[i,j,0,0] * 64 + start_width)
                    y_center = (boxes[i,j,0,1] * 64 + start_height)

    ...
    def build_network(self,
        ...
        with tf.variable_scope(scope):
            with slim.arg_scope([slim.conv2d],
                                ...
                                weights_initializer=tf.truncated_normal_initializer(1, 100)):
                                #weights_regularizer=slim.l2_regularizer(0.0005)):
                #net = tf.pad(images, np.array([[0, 0], [2, 2], [2, 2], [0, 0]]), name='pad_1')
                net = slim.conv2d(images, 64, 7, 2, padding='SAME', scope='conv_2')
                self.net1 = net
                net = leaky_relu(net, alpha=0.1)
                ...
                net = leaky_relu(net, alpha=0.1)
                net = slim.conv2d(net, 256, 3, scope='conv_23')
                net = leaky_relu(net, alpha=0.1)
                net = slim.conv2d(net, 128, 1, scope='conv_24')
                net = leaky_relu(net, alpha=0.1)
                net = slim.conv2d(net, 13, 3, scope='conv_25')
                net = slim.max_pool2d(net, 2, padding='SAME', scope='pool_21')
                net = leaky_relu(net, alpha=0.1)
